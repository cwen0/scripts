...
def run_sysbench(bench, starttime, benchtime, warmuptime):
    drop_cache()
    cmd = ["sh", "-c", "sysbench "+ bench +" --mysql-host=127.0.0.1 --mysql-port=4000 --mysql-user=root --mysql-db=sbtest" +
          " --time="+ str(benchtime) + " --report-interval=10 --db-driver=mysql --tables=32" +
          "--table-size=1000000 --threads=256 --warmup-time="+ str(warmuptime) +" run > sysbench.result"]
    ...
    gen_sysbench_result(bench, starttime)
...
def gen_sysbench_result(bench, starttime):
    tps_cmd = ["sh", "-c", "cat sysbench.result |grep -e 'transactions:'|awk -F['(',' ']+ '{print $4}'"]
    ...
    qps_cmd = ["sh", "-c", "cat sysbench.result |grep -e 'queries::'|awk -F['(',' ']+ '{print $4}'"]
    ...
    lan_avg_cmd = ["sh", "-c", "cat sysbench.result | grep avg: | awk '{print $2}'"]
    ...
    lan_95th_cmd = ["sh", "-c", "cat sysbench.result | grep '95th percentile:' | awk '{print $3}'"]
    ...
    data = {
        ...
        "bench_result": {
            "tps": float(tps_result.stdout),
            "qps": float(qps_result.stdout),
            "lantency_avg": float(lan_avg_result.stdout),
            "lantency_95th": float(lan_95th_result.stdout),
            "time_elapsed": 3600
        }
    }
...
def main():
    ...
    run_sysbench(args.bench, args.starttime, args.benchtime, args.warmuptime)
